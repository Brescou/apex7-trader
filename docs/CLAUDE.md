...
| `dashboard/` | Dash layout, callbacks, `_agent_loop` background thread, Terminal extensions |
...
| `agents/shared/nodes.py` | Shared nodes: `load_memory`, `fetch_data`, `risk_check`, `execute`, `save_memory`, `skip`, `research` |
...
A third daemon thread (`apex7-postmortem`) runs in `dashboard/` and calls `run_daily_postmortem()` once per day at `POSTMORTEM_HOUR`. It only reads `portfolio.trade_history` and writes to SQLite — no Portfolio mutations.
...
The `_llm()` helper in `agents/simple.py` handles the agentic web-search tool loop (up to 8 iterations) using Claude's `web_search_20250305` tool directly via the Anthropic SDK.
...
- **`trades.db` soft migration** — on startup, `agents/simple.py` tries `ALTER TABLE trades ADD COLUMN source TEXT DEFAULT 'live'` and silently catches the error if the column exists. Do not remove this block.
...
- **`start_agent()` in `agents/simple.py` is unused from the dashboard** — `dashboard/` runs its own `_agent_loop` directly. The function exists for standalone use.
- **Postmortem thread only in `dashboard/`** — `run_daily_postmortem()` is never called from `main.py` or `agents/simple.py`. It only runs when the full Dash app is started.
- **`agent_memory` inserts in live path only for specialist nodes** — the simple graph does not write to `agent_memory` at all.
- **Multi-symbol position limit** — `Portfolio.buy()` silently returns `None` (not a dict) if the symbol is already held. Ensure any new callers handle a `None` return gracefully.
...
- **`dashboard/__init__.py` must import all callback modules** — `live`, `analytics`, `backtest_tab`, `terminal`, `history`, `heatmap` must all be imported in `dashboard/__init__.py`. If any are missing, those `@app.callback` decorators are never registered and the corresponding UI updates silently fail.
